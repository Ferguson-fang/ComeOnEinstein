...
//                    mFlingView = (ViewGroup) getChildAt(mPosition - ((LinearLayoutManager) getLayoutManager()).findFirstVisibleItemPosition());
                    ...
                    if (view != null && mFlingView !== view && view.scrollX != 0) {
                        view.scrollTo(0, 0)
                        Log.e("aaa", "onInterceptTouchEvent() -> ACTION_DOWN 拦截事件-关闭已打开的menu")
                        return true
                    }
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (Math.abs(xVelocity) > SNAP_VELOCITY && Math.abs(
                        xVelocity
                    ) > Math.abs(yVelocity)
                    || Math.abs(x - mFirstX) >= mTouchSlop
                    && Math.abs(x - mFirstX) > Math.abs(y - mFirstY)
                ) {
                    Log.e("bbb", "onInterceptTouchEvent() -> ACTION_MOVE true")
                    mIsSlide = true
                    stateCallback!!.dragEnable(false)
                    return true
                } else {
                ...
                MotionEvent.ACTION_UP -> {
                    if (mMenuViewWidth != INVALID_CHILD_WIDTH) {
                        ...
                        if (mVelocityTracker!!.xVelocity < -SNAP_VELOCITY) {    // 向左侧滑达到侧滑最低速度，则打开
                            val delt = Math.abs(mMenuViewWidth - scrollX)
